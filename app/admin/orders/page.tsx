...
export default function AdminOrdersPage() {
  ...
  const displayedOrders = useMemo(() => {
    return orders.filter((order) => {
      ...
      const dateMatch = !dateFilter || order.createdAt.slice(0, 10) === dateFilter;
  ...
  const handleExportCsv = () => {
    const header = ["Order ID", "Customer", "Phone", "Total", "Status", "Created At"];
              ...
              <tbody>
                {displayedOrders.map((order) => (
                  <tr key={order.id} className="border-t border-[#f0e4da]">
                    ...
                    <td className="py-3">
                      <select
                        className="rounded-xl border border-[#e6d8ce] px-2 py-1 text-xs"
                        value={order.status}
                    ...
                    <td className="py-3 text-xs text-muted">{new Date(order.createdAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
