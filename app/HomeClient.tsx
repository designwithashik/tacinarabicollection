...
export default function HomePage({
  ...
  useEffect(() => {
    ...
    const animateProgress = () => {
      const next =
        ...
        (progressTargetRef.current - progressCurrentRef.current) * easing;

      progressCurrentRef.current = next;
      setScrollProgress(next);

      if (Math.abs(progressTargetRef.current - next) > epsilon) {
        progressRafRef.current = window.requestAnimationFrame(animateProgress);
      } else {
        progressCurrentRef.current = progressTargetRef.current;
        setScrollProgress(progressTargetRef.current);
        progressRafRef.current = null;
      }
    };

    const queueProgressUpdate = () => {
      progressTargetRef.current = getProgress();
  ...
  return (
    ...
    >
      ...
      <header
        className={clsx(
          "sticky top-0 z-50 w-full border-b border-[var(--border-soft)] bg-white/95 backdrop-blur-sm transition-shadow duration-300",
          scrollProgress > 0.02 && "shadow-[0_4px_14px_rgba(0,0,0,0.05)]",
        )}
      >
        ...
        >
          <div className="flex h-full items-center justify-center gap-3">
            ...
            <button
              type="button"
              onClick={scrollToTop}
              className="interactive-feedback inline-flex items-center justify-center rounded-lg px-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--bar-maroon)] focus-visible:ring-offset-2"
            ...
            <button
              ...
              className="interactive-feedback absolute right-4 flex h-10 w-10 items-center justify-center rounded-full text-xl text-ink transition-colors hover:bg-black/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--bar-maroon)] focus-visible:ring-offset-2"
